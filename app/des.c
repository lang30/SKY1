...
unsigned char code initial_tr[64] =      //初始置换
{
...
unsigned char code final_tr[64] =
{
...
unsigned char code rots[16] =
{
  1,  1,  2,  2,  2,  2,  2,  2,  1,  2,  2,  2,  2,  2,  2,  1
};

unsigned char code bit_msk[8] =
{
...
/************************************************************************/
void transpose(unsigned char *idate, unsigned char *odate, const unsigned char *tbl, unsigned char n)
{
    ...
    {
        bi_idx = *tbl++;
        if (idate[bi_idx>>3] & tab_adr[bi_idx & 7])
        {
            odate[i>>3] |= tab_adr[i & 7];
        }
    }
    while (++i < n);
}
...
/*      Last changed by:        YXH                                     */
/*      Date:                   2012-07-13                              */
/*      Functional Description: The cipher function                    */
/*      input parameter 1:      pointer to first byte in key string     */
